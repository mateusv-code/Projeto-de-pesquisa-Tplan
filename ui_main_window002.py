...
class ui_MainWindow(object):
    ...
    def widgets_panel_3(self):
        ...
        self.label_adotada =  QLabel()
        self.label_adotada.setStyleSheet('background-color: white')
        self.cota_layout.addWidget(self.label_adotada, 1, 1)  
    ...
    def plataforma(self):
        ...
        self.label_calculada.setText(f'{cota_plataforma:.3f}')

        # --- pega dx (necessário para converter % -> variação de cota) ---
        try:
            dx = float(self.dimensao_X.text())
        except Exception:
            print("Erro: dimensao X (dx) inválida. Preencha DIMENSÃO EM X.")
            return
        ...
        if colunas == 1:
            for _ in range(linhas):
                self.cotas_plataforma_mista.append([cota_plataforma])
        ...
        for i in range(linhas):
            linha_plataforma = [0.0] * colunas
            cota_base = cota_plataforma
        ...
        # debug opcional
        print(f"Gerado cotas_plataforma_mista: linhas={linhas}, colunas={colunas}, len(inclinacoes)={len(inclinacoes)}")
    ...
    def calcular_volumes(self):
        # self.valores_inclinacao = []
        # for i in range(colunas-1):
        #     text_inclinacao = self.list_inclinacao[i].text()
        #     try:
        #         inclinacao = float(text_inclinacao)
        #         self.valores_inclinacao.append(inclinacao)
        #     except ValueError:
        #         print('valores inválidos')
        #         return

        #print(self.valores_inclinacao)
        try:
            dx = float(self.dimensao_X.text())
        ...
        # Percorre seções longitudinais (linhas)
        for i in range(self.linhas):
            area_corte_secao = 0
            area_aterro_secao = 0
            ...
            for j in range(self.colunas - 1):
                ...
                if h1 >= 0 and h2 >= 0:
                    # Todo em corte
                    area_corte = ((h1 + h2) / 2) * dx * fator
                    area_corte_secao += area_corte

                elif h1 <= 0 and h2 <= 0:
                    # Todo em aterro
                    area_aterro = ((abs(h1) + abs(h2)) / 2) * dx * fator
                    area_aterro_secao += area_aterro
                    ...
                    if h1 > 0:
                        ...
                        area_corte = (h1 * x_intersec) / 2 * fator
                        area_aterro = (abs(h2) * (dx - x_intersec)) / 2 * fator
                    else:
                        # ponto inicial em aterro
                        area_aterro = (abs(h1) * x_intersec) / 2 * fator
                        area_corte = (h2 * (dx - x_intersec)) / 2 * fator

                    area_corte_secao += area_corte
                    area_aterro_secao += area_aterro
            area_corte_secao_u.append(area_corte_secao)
            area_aterro_secao_u.append(area_aterro_secao)

            # fim da seção i → salva os volumes dessa seção
            # Adicione as listas de sub-áreas calculadas para esta seção
            self.volumes_de_corte_secoes.append(area_corte_secao_u)
            self.volumes_de_aterro_secoes.append(area_aterro_secao_u)

            self.volume_corte += area_corte_secao
            self.volume_aterro += area_aterro_secao

        # print(f"Volume de Corte: {self.volume_corte:.2f} m³")
        # print(f"Volume de Aterro: {self.volume_aterro:.2f} m³")

        # Atualiza a label da tela
        self.label_adotada.setText(f"{self.volume_aterro:.2f} m³")
        # print(f'{self.volumes_de_aterro_secoes[0]}')
        # print(f'{self.volumes_de_corte_secoes}')


    def abrir_janela_resumo_volumes(self):
        #if self.segunda_janela is None:
        self.segunda_janela = SegundaJanela(volume_aterro=self.volume_aterro, volume_corte=self.volume_corte, linhas= self.linhas, volume_aterro_secoes=self.volumes_de_aterro_secoes, volume_corte_secoes=self.volumes_de_corte_secoes)
